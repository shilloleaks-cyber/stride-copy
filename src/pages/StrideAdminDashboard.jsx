...
export default function StrideAdminDashboard() {
  ...
  // If no event_id, redirect to event list
  ...
  if (!eventIdParam || (!event && events.length > 0)) {
    return (
      <div style={{ minHeight: '100dvh', background: BG, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 12, padding: 24 }}>
        ...
        <p style={{ fontSize: 16, fontWeight: 800, color: '#fff', margin: 0 }}>No event selected</p>
        <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.4)', margin: 0, textAlign: 'center' }}>Go to the Events list to select an event to manage.</p>
        <button
          onClick={() => navigate('/AdminEvents')}
          style={{ padding: '12px 24px', borderRadius: 12, background: ACCENT, color: '#050f08', fontSize: 13, fontWeight: 800, border: 'none', cursor: 'pointer' }}
        >
          Go to Events
        </button>
      </div>
    ...
    <div style={{ minHeight: '100dvh', background: BG, paddingBottom: 100 }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 12 }}>
          ...
          <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.6)', fontWeight: 700, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 180 }}>{event.title}</span>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: 14 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
              <h1 style={{ fontSize: 17, fontWeight: 900, color: '#fff', margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{event.title}</h1>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '3px 12px', marginTop: 4 }}>
              {event.event_date && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                  ...
                  <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.45)', fontWeight: 600 }}>{format(new Date(event.event_date), 'MMM d, yyyy')}</span>
                </div>
              )}
              {event.location_name && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                  <MapPin style={{ width: 11, height: 11, color: 'rgba(0,230,118,0.5)' }} />
                  <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.45)', fontWeight: 600 }}>{event.location_name}</span>
                </div>
              )}
            </div>
          </div>
        ...
        {/* Workspace tabs */}
        <EventWorkspaceTabs activeTab={activeTab} onTabChange={setActiveTab} badges={tabBadges} />
      </div>
